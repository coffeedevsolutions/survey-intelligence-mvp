...
import { 
  ...
  updateSystem,
  deleteSystem,
  getCapabilitiesBySystem,
  getCapabilitiesByOrg,
  createCapability,
  updateCapability,
  deleteCapability,
  getCapabilitySynonyms,
  addCapabilitySynonym,
  deleteCapabilitySynonym,
  getStackPolicies,
  createStackPolicy,
  updateStackPolicy,
  deleteStackPolicy,
...
router.post('/orgs/:orgId/systems', requireMember('admin'), async (req, res) => {
  ...
  try {
    ...
    const system = await createSystem(orgId, systemData);
...
router.put('/orgs/:orgId/systems/:systemId', requireMember('admin'), async (req, res) => {
  try {
    ...
    const system = await updateSystem(systemId, orgId, {
      name: name.trim(),
      vendor: vendor?.trim() || null,
      category: category?.trim() || null,
      status: status || 'active',
...
router.delete('/orgs/:orgId/systems/:systemId', requireMember('admin'), async (req, res) => {
  try {
    ...
    const deleted = await deleteSystem(systemId, orgId);
