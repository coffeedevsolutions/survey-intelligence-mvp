...
import { DEFAULT_FLOW_SPEC, DEFAULT_TEMPLATE } from '../../surveys/services/adaptiveEngine.js';
...
router.get('/orgs/:orgId/campaigns', requireMember('reviewer', 'admin'), async (req, res) => {
  try {
    ...
    const campaigns = await campaignRepository.getCampaignsByOrg(orgId);
    res.json({ campaigns });
  } catch (error) {
...
router.get('/orgs/:orgId/campaigns/archived', requireMember('admin'), async (req, res) => {
  try {
    ...
    const campaigns = await campaignRepository.getArchivedCampaigns(orgId);
...
router.get('/orgs/:orgId/campaigns/:campaignId', requireMember('reviewer', 'admin'), async (req, res) => {
  try {
    ...
    const campaign = await campaignRepository.getCampaignById(campaignId, orgId);
    if (!campaign) {
...
router.put('/orgs/:orgId/campaigns/:campaignId', requireMember('admin'), async (req, res) => {
  try {
    ...
    const campaign = await campaignRepository.updateCampaign(campaignId, orgId, updates);
    if (!campaign) {
      return res.status(404).json({ error: 'Campaign not found' });
    }
...
router.post('/orgs/:orgId/campaigns/:campaignId/flows', requireMember('admin'), async (req, res) => {
  try {
    ...
    const flow = await surveyFlowRepository.createSurveyFlow({
      campaignId,
      title: title || `Flow v${Date.now()}`,
      specJson: spec,
...
router.get('/orgs/:orgId/campaigns/:campaignId/flows', requireMember('reviewer', 'admin'), async (req, res) => {
  try {
    ...
    const flows = await surveyFlowRepository.getFlowsByCampaign(campaignId);
    res.json({ flows });
  } catch (error) {
...
router.get('/orgs/:orgId/flows/:flowId', requireMember('reviewer', 'admin'), async (req, res) => {
  try {
    ...
    const flow = await surveyFlowRepository.getFlowById(flowId);
    if (!flow) {
...
router.post('/orgs/:orgId/campaigns/:campaignId/flows/:flowId/links', requireMember('admin'), async (req, res) => {
  try {
    ...
    const link = await surveyLinkRepository.createSurveyLink({
      orgId,
      campaignId,
      flowId,
    ...
    res.json({
      ...link,
      url: `${apiUrl}/api/ai-survey/bootstrap/${link.token}`
    });
  } catch (error) {
...
router.get('/orgs/:orgId/links', requireMember('admin'), async (req, res) => {
  try {
    ...
    const links = await surveyLinkRepository.getSurveyLinksByOrg(orgId);
    res.json({ links });
  } catch (error) {
...
router.post('/orgs/:orgId/links/:linkId/revoke', requireMember('admin'), async (req, res) => {
  try {
    ...
    const link = await surveyLinkRepository.revokeSurveyLink(linkId, orgId);
    if (!link) {
...
router.get('/orgs/:orgId/campaigns/:campaignId/responses', requireMember('reviewer', 'admin'), async (req, res) => {
  try {
    ...
    const responses = await campaignRepository.getCampaignResponses(campaignId, orgId);
...
router.post('/orgs/:orgId/campaigns/:campaignId/archive', requireMember('admin'), async (req, res) => {
  try {
    ...
    const campaign = await campaignRepository.archiveCampaign(campaignId, orgId);
...
router.post('/orgs/:orgId/campaigns/:campaignId/restore', requireMember('admin'), async (req, res) => {
  try {
    ...
    const campaign = await campaignRepository.restoreCampaign(campaignId, orgId);
    res.json({ success: true, campaign });
  } catch (error) {
...
router.delete('/orgs/:orgId/campaigns/:campaignId/permanent', requireMember('admin'), async (req, res) => {
  try {
    ...
    const campaign = await campaignRepository.deleteCampaignPermanently(campaignId, orgId);
    res.json({ success: true, campaign });
  } catch (error) {
...
router.post('/orgs/:orgId/sessions/:sessionId/archive', requireMember('admin'), async (req, res) => {
  try {
    ...
    const session = await sessionRepository.archiveSession(sessionId, orgId);
...
router.post('/orgs/:orgId/sessions/:sessionId/restore', requireMember('admin'), async (req, res) => {
  try {
    ...
    const session = await sessionRepository.restoreSession(sessionId, orgId);
    res.json({ success: true, session });
  } catch (error) {
...
router.delete('/orgs/:orgId/sessions/:sessionId/permanent', requireMember('admin'), async (req, res) => {
  try {
    ...
    const session = await sessionRepository.deleteSessionPermanently(sessionId, orgId);
    res.json({ success: true, session });
  } catch (error) {
