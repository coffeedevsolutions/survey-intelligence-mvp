...
router.post('/orgs/:orgId/solutions/generate', requireMember('reviewer', 'admin'), async (req, res) => {
  try {
    console.log('🚀 [SOLUTION GENERATION] Starting solution generation...');
    console.log('🚀 [SOLUTION GENERATION] Request body:', req.body);
    console.log('🚀 [SOLUTION GENERATION] Request params:', req.params);
    console.log('🚀 [SOLUTION GENERATION] User:', req.user);
    ...
    if (parseInt(req.user.orgId) !== parseInt(orgId)) {
      console.log('❌ [SOLUTION GENERATION] Access denied - org mismatch');
      return res.status(403).json({ error: 'Access denied' });
    }

    if (!briefId) {
      console.log('❌ [SOLUTION GENERATION] Missing briefId');
    ...
    console.error('❌ [SOLUTION GENERATION] Error generating solution:', error);
    console.error('❌ [SOLUTION GENERATION] Error stack:', error.stack);
    console.error('❌ [SOLUTION GENERATION] Error details:', {
